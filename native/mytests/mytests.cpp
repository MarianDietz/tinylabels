...
size_t w = 512;  // needs to be a power of 2
size_t l = 9;    // = log_2 w
size_t m = 4;    // s.t. g^m > modulus
size_t g = 1 << 14;
...
public:
    ...
    */
    void setup() {
        ...
        SEAL_ITERATE(PolyIter(data_b_.get(), poly_modulus_degree, coeff_modulus_size), w, [&](const RNSIter &I) {
            sample_poly_uniform(prng, parms, I);
        });
        // no need to convert a into NTT, as we may assume that it was already sampled in NTT form
    }
...
int main()
{
    ...
    Pointer<uint64_t> l1(allocate_zero_uint(w*poly_modulus_degree, pool));
    Pointer<uint64_t> l2(allocate_zero_uint(w*poly_modulus_degree, pool));
    Pointer<uint64_t> out(allocate_zero_uint(w*poly_modulus_degree, pool));
    ...
    Pointer<uint64_t> m2(allocate_zero_poly_array(w, poly_modulus_degree, coeff_modulus.size(), pool));
    PolyIter m2Iter(m2.get(), poly_modulus_degree, coeff_modulus.size());
    Pointer<uint64_t> a(allocate_zero_poly_array(w, poly_modulus_degree, coeff_modulus.size(), pool));
    RNSIter aIter(a.get(), poly_modulus_degree);

    s[4] = 2;
    m2[4] = 1;
    a[4] = 3;

    Lenc lenc(context_data, prng);
    LHE lhe(context_data, prng);
    lenc.setup();
    lhe.setup();

    Pointer<uint64_t> &r = lenc.enc(s);
    lhe.enc1(r);
    lhe.enc2(m2);

    Pointer<uint64_t> &y = lenc.digest(a);
    lhe.keygen(y);

    Pointer<uint64_t> &delta = lenc.eval(a);
    Pointer<uint64_t> &mres = lhe.dec(y);

    PolyIter deltaIter(delta.get(), poly_modulus_degree, coeff_modulus.size());
    PolyIter mresIter(mres.get(), poly_modulus_degree, coeff_modulus.size());
    sub_poly_coeffmod(mresIter, deltaIter, w, coeff_modulus, mresIter);

    //cout << poly_to_hex_string(y.get(), poly_modulus_degree, 1) << "\n";//, pool);
    //cout << poly_to_hex_string(y.get() + poly_modulus_degree, poly_modulus_degree, 1) << "\n";//, pool);
    ...
    cout << poly_to_hex_string(ct1.get(), 6, 1) << " ct1\n";//, pool);
    ...
    cout << poly_to_hex_string(ct2.get(), 6, 1) << " ct2\n";//, pool);
    ...
    cout << poly_to_hex_string(ctres.get(), 6, 1) << " ctres\n";//, pool);
